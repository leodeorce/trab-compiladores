...
double int_times(double l, double r) {
    return l * r;
}
// *******************************************
...
void debug(AST *ast, char* kid_name){
    printf("Criar ma função para resolver esse caso ->>", kid_name);
}
...
void run_over_2(AST *ast) {
    trace("over");
    run_other_arith_2(ast, number_over);
}
// ---------------- =-------------------------
...
void run_read(AST *ast) {
    ...
    Type var_type = get_type(vt, var_idx);
    switch(var_type) {
        case NUMBER_TYPE:       number_int(var_idx);   break;
...
void rec_run_ast(AST *ast) {
    switch(get_kind(ast)) {
        ...
        //-----------------Tem que implementar--------------------
        case BEGIN_NODE:            debug(ast, "BEGIN_NODE");   break;  
        case DIV_NODE:              debug(ast, "DIV_NODE");     break; 
        ...
        case LT_NODE:               debug(ast, "LT_NODE");      break; 
        case MULT_NODE:             debug(ast, "MULT_NODE");    break; 
        case NUM_VAL_NODE:          debug(ast, "NUM_VAL_NODE"); break; 
        case OR_NODE:               debug(ast, "OR_NODE");      break; 
        case PLUS_NODE:             debug(ast, "PLUS_NODE");    break; 

        case  PRINT_NODE:           debug(ast, "PRINT_NODE");   break;
        case  SUB_NODE:             debug(ast, "SUB_NODE");     break;
        case  STR_VAL_NODE:         debug(ast, "STR_VAL_NODE"); break;
        case  VAR_DECL_NODE:        debug(ast, "VAR_DECL_NODE");break;
        case  VAR_USE_NODE:         debug(ast, "VAR_USE_NODE"); break;
        case  WHILE_NODE:           debug(ast, "WHILE_NODE");   break;

        case  B2S_NODE:             debug(ast, "B2S_NODE");     break;
        case  B2U_NODE:             debug(ast, "B2U_NODE");     break;
        case  N2S_NODE:             debug(ast, "N2S_NODE");     break;
